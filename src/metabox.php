...
function mytheme_blocks_post_options_metabox_html($post)
{
    ...
    wp_nonce_field(
        'mytheme_blocks_update_post_metabox',
        'mytheme_blocks_update_post_nonce'
    );
?>
    <p>
        <label for="mytheme_blocks_post_subtitle_field"><?php esc_html_e(
                                                            'Post Subtitle',
                                                            'mytheme_blocks'
                                                        ); ?></label>
        <br />
        <input class="widefat" type="text" name="mytheme_blocks_post_subtitle_field" id="mytheme_blocks_post_subtitle_field" value="<?php echo esc_attr(
                                                                                                                                        $subtitle
                                                                                                                                    ); ?>" />
    </p>
<?php
...
function mytheme_blocks_save_post_metabox($post_id, $post)
{
    ...
    if (
        !isset($_POST['mytheme_blocks_update_post_nonce']) ||
        !wp_verify_nonce(
            $_POST['mytheme_blocks_update_post_nonce'],
            'mytheme_blocks_update_post_metabox'
        )
    ) {
        return;
    ...
    if (array_key_exists('mytheme_blocks_post_subtitle_field', $_POST)) {
        update_post_meta(
            ...
            '_mytheme_blocks_post_subtitle',
            sanitize_text_field(
                $_POST['mytheme_blocks_post_subtitle_field']
            )
        );
    }
}
